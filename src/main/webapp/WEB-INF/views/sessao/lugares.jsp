							...
							<table>
								...
								<c:forEach var="lugar" items="${map.value}">
									<td class="fileira-assento"><figure>
										<svg class="assento ${sessao.isDisponivel(lugar) && !carrinho.isSelecionado(lugar, sessao) ? 'disponivel' : 'ocupado'}" id="${lugar.id}"  version="1.0" id="SEAT" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
												 onclick="${sessao.isDisponivel(lugar) && !carrinho.isSelecionado(lugar, sessao) ? 'changeCheckbox(this)': ''}" viewBox="0 0 318.224 305.246" enable-background="new 0 0 318.224 305.246" xml:space="preserve">
											<g id="FILL">
